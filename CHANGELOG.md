...
- **An OpenCode qualification kit** (v0.30.0). Adds the tooling needed to decide whether a gateway and model are usable as an OpenCode agent backend, before any runtime integration exists: an offline fake OpenAI-compatible provider for exercising the checks without a real gateway, a standard-library gateway probe that runs a short handshake and writes a capability record (exit 0 qualified, 1 not qualified, 2 could not run), a compatibility document pinning one OpenCode release with per-claim verification status, and a maintainer decisions document listing every configuration choice only a maintainer can make. No OpenCode runtime integration ships yet.

- **claude-code-router v3 support: quoin reads and writes CCR's sqlite config store** (v0.29.0). CCR v3 keeps its configuration in `~/.claude-code-router/config.sqlite` and no longer reads the v2 `config.json`, so `quoin router setup`, `quoin models set` and `quoin models preset` were writing a file their CCR would never open. A new standard-library-only store module reads the store's configuration blob, merges into it, and writes it back, taking a value-level backup of the previous blob beside the store before the merge — the backup path is printed on every write, so any change can be undone by hand. On a machine that looks like a v2 to v3 upgrade that lost its configuration — a leftover `config.json` beside an empty store — `quoin router setup` and `quoin router status` say so and name what was lost. Two situations are declined rather than guessed at: a CCR major quoin does not recognise, and a v3 install whose store does not exist yet (run `ccr start` once to have CCR create it). Nothing is written to CCR in either case. Launch guidance is now version-aware everywhere it appears, including the AgentDesk CCR pane, because v3 has no `ccr code` subcommand; `ccr -v` and `ccr version` are deliberately never invoked, since a version query on v3 migrates and removes `config.json`. The v2 `background`, `think` and `longContext` routes have no v3 equivalent and are deliberately not translated into rules that would only look equivalent; every v3 write says so.
...
- **A claude-code-router 1.x install with a stray, zero-byte `config.sqlite` no longer gets routed into a sqlite write it can't read back** (v0.29.3). The previous fix scoped the empty-store fall-through to the one npm major a legacy `config.json` store is ever live for, which left a 1.x install with no arm able to object to a real database materializing inside what may just be leftover bytes — and, on a leftover-`config.json` machine, a false report that an upgrade had wiped the configuration. The fall-through is back to covering any npm major that disagrees with v3, matching what a v3-store-absent install has always needed; the decline message now names a zero-byte store explicitly when one is present instead of implying no store exists at all. Also fixed: the test-isolation guard checked `os.rename` and `os.replace` on their destination only, so a test moving the real CCR store away — rather than writing onto it — went unguarded despite the guard's own comment and changelog claiming otherwise; both directions are checked now. `os.symlink` and `os.link` are wrapped by the same guard, since either can place a `config.json` or `config.sqlite` file at the store path and change what detection reports.
- **`quoin router status`, `models show` and the setup path go back to a plain, bounded `npm prefix -g` spawn instead of guessing npm's prefix from its own binary location** (v0.29.2). The guess traded a small latency win for an unbounded `~/.npmrc` read on every CCR-touching command: reading a FIFO or device node through it never returns, and an oversized or binary file gets materialized into memory whole. Rather than patch the guess again, it's removed outright — detection is back to the same `timeout=10`-bounded spawn quoin used before the optimisation existed, with the same fail-closed-to-`None` behavior on a slow or missing npm. Also fixed: the fall-through that lets a disagreeing npm major override an ambiguous, zero-byte `config.sqlite` used the wrong major as its trigger, routing a claude-code-router 1.x install into a "store not created yet" message naming a store file that was visibly present; it's now scoped to the one major a legacy `config.json` store is ever live for. A detection call that hits this fall-through with no npm major already supplied now reads npm once instead of twice. The test-isolation guard that keeps the suite from touching a real CCR install now also wraps `os.rename`, `os.rmdir` and `os.truncate`, closing the remaining seams that could move or zero a real store undetected.
- **CCR version detection no longer trusts an npm-optimised fast path or an empty store over a real, disagreeing signal** (v0.29.1). The `npm prefix -g` fast path added alongside the v0.29.0 CCR v3 support guessed npm's global prefix from its own binary location and accepted any CCR package found there, without checking whether npm's prefix had been overridden elsewhere (`npm config set prefix`, npm's own documented remedy for a global-install permission error) — a leftover CCR package sitting at the binary-adjacent prefix could then be reported instead of the real one, and quoin's `router setup`/`models` surfaces would act on the stale reading. The fast path now falls through to the real `npm prefix -g` spawn whenever a prefix override is configured (`npm_config_prefix`/`NPM_CONFIG_PREFIX`, or a `prefix=` line in `~/.npmrc` or the guessed prefix's own `etc/npmrc`). Separately, a zero-byte `config.sqlite` — indistinguishable by content from a real, freshly-initialised v3 store, since sqlite treats an empty file as a valid empty database — is no longer treated as conclusive v3 evidence on a machine npm gives a definite, disagreeing reason to doubt it; detection now falls through to the npm/`config.json` signal in that case, so a genuine v2 machine with an unrelated empty file at that path no longer gets misreported as a v2-to-v3 upgrade that lost its configuration. Also fixed in the same pass: `--dry-run` could report success for a write the real run would refuse (a non-finite numeric value); the check is now pure and runs before the dry-run return, which also means the write no longer strands a key-bearing backup sidecar on that refusal. Three CLI-wiring tests resolved the real `~/.claude-code-router/` instead of a temp directory, so their result depended on whatever CCR store happened to exist on the machine running the suite; they now take an explicit home override. The test-isolation guard that keeps the suite from touching a real CCR install now also wraps the delete seams (`os.unlink`, `os.remove`, `shutil.rmtree`).

- **Four small CCR-adjacent inconsistencies fixed together** (v0.29.4). A stray, empty `config.sqlite` reaching the no-store-to-merge-into decline was worded as though CCR were still writing to it; the wording now says the file is stray, since the one machine shape where it could genuinely be a pending v3 store never reaches this decline. The guard that falls through past an empty store when npm disagrees now keys on its own constant instead of reusing the general recognised-major ceiling, matching the equivalent split already in place for the legacy `config.json` store. The test-isolation guard's hard-link wrapper checked only a link's destination, so a hard link created *from* the real CCR store into a test-controlled path was a read escape it missed; the source is checked now too. `quoin doctor`'s open-model probe only checked whether a legacy config file existed, so a v3 machine (whose configuration lives in the newer sqlite store instead) was reported as absent and native even when fully set up; it now detects and renders the version the same way `quoin router status` does.
- **`quoin doctor` and `quoin router status` now agree on the installed CCR major** (v0.29.4). The previous fix in this same release had `doctor` render the raw detected version directly, so a leftover `config.json` sitting beside a newer npm package still made `doctor` and `router status` report different majors for the same machine — the one case the existing npm-corrected reading exists to catch. Both commands now render the version through a single shared helper, so they can no longer drift apart.
...
- **Spend monitor gains a dedicated Fable tier instead of dumping it into "other"** (v0.28.5). The spend monitor's per-model breakdown classified every Fable session into a catch-all `other` bucket, because the tier mapping only tested for `opus`/`sonnet`/`haiku` substrings. Fable is the largest tier by volume in sampled transcripts, so the bucket was materially wrong. It now has its own row, and the classification and display ordering are both derived from one ordered table instead of two hand-maintained lists that had to be kept in sync by hand.
